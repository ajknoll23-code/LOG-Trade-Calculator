...
Usage:
    python3 scripts/diff_snapshots.py scripts/value_snapshots/OLD.json scripts/value_snapshots/NEW.json
...
def main():
    snap_dir = Path('scripts/value_snapshots')
    ...
    if len(sys.argv) == 3:
        ...
        if len(files) < 2:
            print(f"Need at least 2 snapshots in {snap_dir} to diff. "
                  f"Found {len(files)}. Run snapshot_values.py first (and again after your rebuild).")
            sys.exit(1)
