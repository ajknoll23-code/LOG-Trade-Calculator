...
PPG_PATH = SCRIPT_DIR / "ppg_pipeline.py"
PLAYERS_CACHE_PATH = REPO_ROOT / "data" / "players_cache.json"
OUT_PATH = SCRIPT_DIR / "player_positions.json"
...
# Historical labels observed in KTC data that are not general PPG aliases.
KTC_LEGACY_ALIASES = {
...
def load_collision_safe_sleeper_positions(cache_path=PLAYERS_CACHE_PATH):
    ...
    This is fallback-only metadata for historical KTC names that are no longer
    in PLAYER_DB. If the same normalized name maps to more than one position
    bucket, omit it rather than guessing identity.
    """
