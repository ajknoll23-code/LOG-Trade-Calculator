...
MAX_WEEK = 18  # covers regular season + playoffs; extra empty weeks are cheap to check
...
def walk_season_chain(start_league_id):
    """Return [(league_id, league_info), ...] oldest-first, following
    previous_league_id back until it's null."""
    seasons = []
...
def load_players_cache():
    """Reuse the player-name cache written by sync_sleeper.py if present —
    player IDs are stable across seasons, so no need to refetch."""
...
def pull_season_trades(league_id, season, players_pool, owner_map):
    ...
    for week in range(1, MAX_WEEK + 1):
        ...
        for t in txns:
            ...
            if not picks:
                continue  # only care about trades that moved at least one pick
            ...
            for pk in picks:
                # "owner_id" here is the roster_id RECEIVING the pick in this trade
