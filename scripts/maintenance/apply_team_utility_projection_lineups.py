...
def patch_sleeper_id_in_merge_function(text, function_name):
    """Insert stable Sleeper ID into one live-merge function only.

    The personal and league merge functions intentionally have different
    surrounding source, so this must not rely on them sharing one exact
    birth_date->role text template.
    """
    signature = f"function {function_name}(data){{"
    start = text.find(signature)
    if start < 0:
        raise RuntimeError(f"{function_name}: function signature not found")
    ...
    sleeper_rows = [
        ...
        if "sleeper_id:" in line and "p.player_id" in line
    ]
    if len(sleeper_rows) == 1:
        return text
    if len(sleeper_rows) > 1:
        raise RuntimeError(
            f"{function_name}: expected at most 1 existing Sleeper-ID row, "
            f"found {len(sleeper_rows)}"
        )

    lines = block.splitlines(keepends=True)
    birth_indexes = [
        i for i, line in enumerate(lines)
        if "birth_date:" in line and "p.birth_date" in line
    ]
    if len(birth_indexes) != 1:
        raise RuntimeError(
            f"{function_name}: expected exactly 1 birth_date row, "
            f"found {len(birth_indexes)}"
        )

    idx = birth_indexes[0]
    birth_line = lines[idx]
    indent = birth_line[: len(birth_line) - len(birth_line.lstrip())]
    newline = "\r\n" if birth_line.endswith("\r\n") else "\n"
    lines.insert(
        idx + 1,
        indent
        + "sleeper_id: (p.player_id !== undefined && p.player_id !== null) "
        + "? String(p.player_id) : (existing ? (existing.sleeper_id || null) : null),"
        + newline,
    )
...
def _selftest_sleeper_id_merge_matcher():
    fixture = """function mergeLiveRoster(data){
  PLAYER_DB[key] = {
...
function mergeLeagueRosters(data){
  PLAYER_DB[key] = {
    birth_date: (typeof p.birth_date === 'string') ? p.birth_date : null,
    // Extra league-only commentary intentionally breaks adjacency.
    role: existing ? existing.role : slotDefaults[slot],
  };
}
"""
    out = patch_sleeper_id_in_merge_function(fixture, "mergeLiveRoster")
    out = patch_sleeper_id_in_merge_function(out, "mergeLeagueRosters")
    assert out.count("sleeper_id:") == 2, out

    again = patch_sleeper_id_in_merge_function(out, "mergeLiveRoster")
    again = patch_sleeper_id_in_merge_function(again, "mergeLeagueRosters")
    assert again == out

    print("PASS independent Sleeper-ID merge matcher self-test")
...
""",
    ...
    # Patch each live-merge function independently. Their surrounding source
    # is allowed to differ; only that function's own birth_date row is used
    # as the insertion anchor.
    text = patch_sleeper_id_in_merge_function(text, "mergeLiveRoster")
    text = patch_sleeper_id_in_merge_function(text, "mergeLeagueRosters")
...
def selftest():
    _selftest_sleeper_id_merge_matcher()
    build()
