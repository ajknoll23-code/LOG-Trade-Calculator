...
def run_selftest() -> None:
    ...
    # Full in-memory deployment dry run against the actual checkout.
    # Nothing is written. This must succeed before --apply is allowed to run.
    required = (
        INDEX,
        SYNC_SLEEPER,
        SNAPSHOT,
    ...
    if all(path.exists() for path in required):
        ...
        rb_birth_map, _ = build_rb_birth_map(
            player_db,
            pool,
            as_of=datetime.now(timezone.utc).date(),
        )

        generated_sync = patch_sync_sleeper(sync_text)
        generated_index = patch_index(index_text, rb_birth_map)
        generated_snapshot = patch_snapshot(snapshot_text)
        generated_regression = patch_regression(regression_text)
        ...
        for label, source in (
            ("generated scripts/sync/sync_sleeper.py", generated_sync),
            ("generated scripts/validation/snapshot_values.py", generated_snapshot),
            (
                "generated scripts/validation/repo_regression_checks.py",
                generated_regression,
            ),
        ):
            compile(source, label, "exec")

        assert "def safe_player_birth_date(" in generated_sync
        assert '\\"\\"\\"' not in generated_sync
        assert "RB_CONTINUOUS_AGE_V1" in generated_index
        assert len(rb_birth_map) >= MIN_RB_DOB_COVERAGE
...
def apply_patch() -> None:
    ...
    # Hard gate: never write generated Python that does not parse.
    for label, source in (
        ("scripts/sync/sync_sleeper.py", patched_sync),
        ("scripts/validation/snapshot_values.py", patched_snapshot),
        ("scripts/validation/repo_regression_checks.py", patched_regression),
    ):
        try:
            compile(source, label, "exec")
        except SyntaxError as exc:
            die(
                f"Generated {label} failed Python syntax validation: "
