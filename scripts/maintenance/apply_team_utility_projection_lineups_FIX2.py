...
let TEAM_UTILITY_SLEEPER_ID_BY_KEY = {};
let MY_ROSTER_ID = null; // set from live sync; used to exclude my own team from the trade-target scan
let OWN_ROSTER_ID = null; // set ONLY by the personal sync below, never by switchToTeam —
""",
...
""",
      ...
      if(p.player_id !== undefined && p.player_id !== null){
        TEAM_UTILITY_SLEEPER_ID_BY_KEY[key] = String(p.player_id);
      }
    }
  }
  MY_ROSTER = keys;
...
""",
  ...
  for(const p of team.players){
    if(p.sleeper_id) TEAM_UTILITY_SLEEPER_ID_BY_KEY[p.key] = String(p.sleeper_id);
  }
  MY_ROSTER_ID = rosterId;
""",
...
function teamUtilityProjectionByKey(key){
  ...
  const info = PLAYER_DB[key];
  if(!info || info.pos === 'K') return null;
  const sleeperId = TEAM_UTILITY_SLEEPER_ID_BY_KEY[key];
  if(!sleeperId) return null;
  const row = TEAM_UTILITY_LINEUP_PROJECTIONS.players[String(sleeperId)];
  if(!row) return null;
...
def check_team_utility_projection_lineup_invariants():
    ...
    required = (
        ...
        "starterEligible: slot !== 'taxi' && slot !== 'reserve_ir'",
        "preSlotByKey = MY_ROSTER_SLOT_BY_KEY",
        "TEAM_UTILITY_SLEEPER_ID_BY_KEY",
    )
...
def build():
    ...
    # This release must not rewrite the static PLAYER_DB at all. Reuse the
    # exact production parser that the regression suite uses, so any future
    # quote/brace/parser break is caught during --selftest BEFORE --apply.
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "team_utility_snapshot_guard",
        ROOT / "scripts" / "validation" / "snapshot_values.py",
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load snapshot_values.py for PLAYER_DB guard")
    snapshot_guard = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(snapshot_guard)
    ...
    if new_player_body != old_player_body:
        raise RuntimeError(
            "Team Utility starter-objective patch unexpectedly changed static PLAYER_DB"
        )
