...
def check_dual_eligibility_audit():
    rows = json.load(open(SCRIPT_DIR.parent / "artifacts" / "generated" / "dual_eligibility_results.json"))
...
def check_aliases_and_ktc_positions():
    ...
    stored = json.load(open(SCRIPT_DIR.parent / "artifacts" / "generated" / "player_positions.json"))
...
def check_idp_v1_projection_invariants():
    ...
    baseline_path = REPO_ROOT / "model" / "releases" / "idp-v1" / "prod_mult_pre_v1_baseline.json"
...
def check_canonical_history_and_v1_bridge():
    ...
    frozen_history = json.load(open(REPO_ROOT / "model" / "releases" / "idp-v1" / "production_history_components.json"))
    ...
    bridge = json.load(open(REPO_ROOT / "model" / "releases" / "idp-v1" / "idp_v1_model_delta_transport_candidate.json"))
...
def check_preferred_bake_preview_invariants():
    patch_path = REPO_ROOT / "model" / "releases" / "idp-v1" / "idp_v1_prod_mult_patch.json"
    candidate_path = REPO_ROOT / "model" / "releases" / "idp-v1" / "idp_v1_model_delta_transport_candidate.json"
    baseline_path = REPO_ROOT / "model" / "releases" / "idp-v1" / "prod_mult_pre_v1_baseline.json"
