# Package Adjustment V5 — Preregistered Diagnostic Method

**Research only. Production V1.5 remains unchanged.**

The V5 experiment asks whether the two-player consolidation premium should change as a package moves away from the frozen V3 ~51/49 construction.

The primary analysis is deliberately nonparametric. Within each composition band (50/50, 55/45, 60/40, 65/35, 70/30, 75/25), voter-capped package-choice rates are fit with a weighted isotonic curve over the tested raw package/target FV ratios. The 50% indifference ratio is then obtained only by interpolation between tested cells.
