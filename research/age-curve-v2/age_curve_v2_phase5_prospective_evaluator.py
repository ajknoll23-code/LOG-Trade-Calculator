...
def build_frozen_payload() -> dict[str, Any]:
    ...
    # Freeze at the Phase-4 research generation time, which is the evidence state
    # that selected the candidate family. This must be preseason.
    frozen_at = str(phase4.get("generated_at_utc") or "").strip()
    if not frozen_at:
        raise RuntimeError("Phase 4 missing generated_at_utc")
    ...
    payload = {
        ...
        "source_phase4_generated_at_utc": phase4.get("generated_at_utc"),
        "cohort_definition": (
